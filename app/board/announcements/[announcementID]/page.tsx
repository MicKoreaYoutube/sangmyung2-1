        ...
        <InView triggerOnce={true} threshold={1} delay={1000}>
          {({ inView, ref }) => (
            <Card className={`animate__animated ${inView ? classToAdd : "invisible"}`} ref={ref}>
              ...
              <CardHeader>
                <CardTitle className="font-KBO-Dia-Gothic_bold text-2xl md:text-4xl">{announcement?.title}</CardTitle>
                <CardDescription className="font-SUITE-Regular text-md flex flex-col justify-between md:flex-row md:text-xl">
                  <span>{announcement?.author}</span>
                  <span className="md:text-end">{announcement?.updateTime.toDate().toLocaleString()}</span>
                </CardDescription>
              </CardHeader>
              <CardContent className="font-SUITE-Regular space-y-3">
                <p className="font-SUITE-Regular whitespace-pre-wrap text-lg">{announcement?.content}</p>
              </CardContent>
